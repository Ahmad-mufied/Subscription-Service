...
func TestConfig_AddDefaultDFata(t *testing.T) {
...
func TestConfig_IsAuthenticated(t *testing.T) {
	...
	if auth {
		t.Error("returns true for authenticated, when is should be false")
	}
	...
	if !auth {
		t.Error("returns true for authenticated, when is should be true")
	}
}
